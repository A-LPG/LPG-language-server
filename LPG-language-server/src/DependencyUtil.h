...
public:
	...
	// 如何判断是否有环：如果 make_sort_result.size() 为 0，意味着有环；
	// 此时如需得知具体环，可调用 getCycles 函数来获取。
	...
	const vector < cycleArray >& getCycles()
	{
		...
		if (_is_DAG) // 查看是否是 DAG；如果为真，则没有环，不需要计算了
			return _Cycles;
...
private:
	...
	bool _consistency; // 有新的边添加到图中，而且没有被计算过，这个值为 false
