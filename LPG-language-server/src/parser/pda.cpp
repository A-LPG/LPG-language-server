...
void Pda::PrintShiftState(int state_no)
{
    ...
    //FillIn(buffer, (PRINT_LINE_SIZE - l), '-');
    ...
    //        if (strlen(line) + NumberLength(previous_state) > PRINT_LINE_SIZE-2)
    //        {
    //            fprintf(option -> syslis,"\n%s",line);
    //            strcpy(line, "  ");
    //        }
...
void Pda::PrintLaState(int state_no)
{
    ...
    //                                     // + 2 spaces + newline
    //FillIn(buffer, (PRINT_LINE_SIZE - i), '-');
...
int Pda::MaximumSymbolNameLength(int state_no)
{
    ...
    return Util::Min(max_size, PRINT_LINE_SIZE - 17);
}
