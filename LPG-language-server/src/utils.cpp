...
std::string GetPathFromFileFullPath(const std::string& strFileFullName)
{
	std::stringex temp = strFileFullName;
	temp.replace('\\', '/');
	auto pos = temp.rfind(("/"));
...
boost::optional<AbsolutePath> GetDefaultResourceDirectory() {
  std::string result;
