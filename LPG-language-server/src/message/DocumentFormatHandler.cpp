    ...
    const lsFormattingOptions& options) :unit(u), out(o)
{
    ...
    ILexStream* lex = unit->_lexer.getILexStream();
    lsTextEdit edit;
    auto pos = ASTUtils::toPosition(lex, unit->root->getLeftIToken()->getStartOffset());
	if(pos)
	{
        edit.range.start = pos.value();
	}
     pos = ASTUtils::toPosition(lex, unit->root->getLeftIToken()->getEndOffset());
    if (pos)
    {
